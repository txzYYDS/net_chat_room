...
基于 C 语言 TCP + select 的聊天室，通过 Node.js 桥接层实现网页端访问，用户数据持久化于 SQLite。
...
| 方式 | 连接路径 | 说明 |
|------|---------|------|
| **TCP 直连** | `telnet 192.168.3.56 8888` | 传统终端客户端，输入名字和密码后收发消息 |
...
| **紫色渐变主题** | 暗色玻璃拟态 + 两面气泡统一紫色渐变（自己偏蓝紫、对方偏紫蓝） |
...
| 2025-06-25 | v7 | 新增用户密码（psk）字段、三级状态机登录流程 |
...
无 — 基于 C 语言 TCP + select 实现，Web 桥接层用 Node.js 手写 WebSocket 协议 + SQLite 用户存储。
